...
const (
	ORDERSCENARIO_LIVE OrderScenario = "live"
	ORDERSCENARIO_SIMULATE_SUCCESS OrderScenario = "simulate-success"
	ORDERSCENARIO_SIMULATE_FAILED OrderScenario = "simulate-failed"
	ORDERSCENARIO_SIMULATE_DELAYED_SUCCESS OrderScenario = "simulate-delayed-success"
	ORDERSCENARIO_SIMULATE_DELAYED_FAILED OrderScenario = "simulate-delayed-failed"
)

// All allowed values of OrderScenario enum
var AllowedOrderScenarioEnumValues = []OrderScenario{
	"live",
	"simulate-success",
	"simulate-failed",
...
func (v *OrderScenario) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := OrderScenario(value)
	for _, existing := range AllowedOrderScenarioEnumValues {
	...
	return fmt.Errorf("%+v is not a valid OrderScenario", value)
}

// NewOrderScenarioFromValue returns a pointer to a valid OrderScenario
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewOrderScenarioFromValue(v string) (*OrderScenario, error) {
	ev := OrderScenario(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for OrderScenario: valid values are %v", v, AllowedOrderScenarioEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v OrderScenario) IsValid() bool {
	for _, existing := range AllowedOrderScenarioEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to OrderScenario value
func (v OrderScenario) Ptr() *OrderScenario {
	return &v
}

type NullableOrderScenario struct {
	value *OrderScenario
	isSet bool
}

func (v NullableOrderScenario) Get() *OrderScenario {
	return v.value
}

func (v *NullableOrderScenario) Set(val *OrderScenario) {
	v.value = val
	v.isSet = true
}

func (v NullableOrderScenario) IsSet() bool {
	return v.isSet
}

func (v *NullableOrderScenario) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOrderScenario(val *OrderScenario) *NullableOrderScenario {
	return &NullableOrderScenario{value: val, isSet: true}
}

func (v NullableOrderScenario) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOrderScenario) UnmarshalJSON(src []byte) error {
	v.isSet = true
